...
Use `median2()` as a drop-in replacement for `median()`. Except for handling missing values, it works like `median()` by default.
...
However, `median_table()` is even more informative. It estimates the median while only ignoring as many missing values as absolutely necessary. This balances the need for knowledge about the central tendency with its degree of uncertainty.
