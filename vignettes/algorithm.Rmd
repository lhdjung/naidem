...
Below, `x` is the input distribution. `length(d)` computes the number of values in a distribution `d`, `sort(d)` is a sorting algorithm, and `d[missing]` indexes all missing values in `d` (but no known values). `NA` denotes a single missing value. Unlike R, these algorithms use zero-indexing. Note that the combination of division and flooring in step 1 of the algorithm for odd lengths is equivalent to integer division.

## For odd-length distributions

``` {eval="FALSE"}
# 1. Determine the central index of `x`.
i_half := floor(length(x) / 2)

# 2. Count the missing values in `x`.
n_na := length(x[missing])
...
if x[i_half_1] == x[i_half_1 - n_na] && x[i_half_2] == x[i_half_2 - n_na]:
   return (x[i_half_1] + x[i_half_2]) / 2
else:
   return NA
```
...
The algorithms return `NA` if any offset central index is negative, i.e., if `i_half(_1) - n_na < 0`. (This is step 3 for odd lengths and step 4 for even lengths. The pseudo-code in the algorithm is more efficient.) In the even-length case, only `i_half_1` needs to be tested because it is less then `i_half_2`, so it is negative whenever `i_half_2` is. Why return `NA` here? The median is never known in such cases, and indices must not be negative when testing for equality in the final condition. Negative indices are not well defined, and programming languages such as R and Python would handle them in unintended ways.

## R details

The R implementation in naidem is more succinct than the pseudo-code. This plays into the strengths of R, but it does make the code look a little different from the above. Again, note that the pseudo-code uses zero-indexing but R uses one-indexing. This is why `any(n_na + 1L > half)` checks for non-positive indices, not just for negative ones.
...
If this doesn't occur and `x` was originally even, the code  uses `mean()` instead of manually dividing the sum of the half-indexed `x` values by 2. That is because some classes may have specific `mean()` methods implemented. Compare this to the details section of `?stats::median()`.
