...
4.  if $i_{half} - n_{NA} < 0$: // Any negative indices lead to a missing value.
...
5.  if $i_{half1} - n_{NA} < 0 \lor i_{half2} - n_{NA} < 0$:
...
Why do the algorithms return $NA$ if any offset central index is negative? (This is step 4 for odd lengths and step 5 for even lengths.) The median is never known in such cases. Practically speaking, however, the more important reason is that indices must not be negative when testing for equality in the final condition. Negative indices are not well defined, and programming languages like R and Python would handle them in unintended ways.
