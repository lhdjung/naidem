...
The R implementation in naidem is more succinct than the above. This plays into the strengths of R, but it makes the code look a little different from the pseudo-code above. Again, note that the pseudo-code uses zero-indexing but R uses one-indexing. This is why `any(nna + 1L > half)` checks for non-positive indices, not just for negative ones.
