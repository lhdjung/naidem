...
Note the use of `near()`, which was adapted from `dplyr::near()` but without depending on dplyr. Consistent with the explanation above, `near()` compares two numeric vectors while disregarding any spurious floating-point differences that `==` would take seriously (see also [*R for Data Science* on this point](https://r4ds.hadley.nz/logicals.html#sec-fp-comparison)). So `near(0.1 + 0.2, 0.3)` is `TRUE` even though `0.1 + 0.2 == 0.3` is `FALSE`. Nevertheless, `==` is used elsewhere in `median2()` when comparing integers, where there is no such problem.
