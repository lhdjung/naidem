...
The algorithms return `NA` if any offset central index is negative, i.e., if `half(_1) - nna < 0`. (This is step 3 for odd lengths and step 4 for even lengths. The pseudo-code in the algorithm is more efficient.) In the even-length case, only `half_1` needs to be tested because it is less then `half_2`, so it is negative whenever `half_2` is. Why return `NA` here? The median is never known in such cases, and indices must not be negative when testing for equality in the final condition. Negative indices are not well defined, and programming languages such as R and Python would handle them in unintended ways.
