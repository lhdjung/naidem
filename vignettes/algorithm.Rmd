...
Below, $x$ is the input distribution. $length(d)$ computes the number of values in a distribution $d$, $sort(d)$ is a sorting algorithm, and $d[missing]$ indexes all missing values in $d$ (but no known values). $NA$ denotes a single missing value. Note that the combination of division and flooring in step 1 of the odd-length algorithm is equivalent to integer division.

## Odd-length distributions

1.  $i_{half} := \lfloor{\frac{length(x) + 1}{2}}\rfloor$ // Determine the central index of $x$.

2.  $n_{NA} := length(x[missing])$ // Count the missing values in $x$.

3.  $x := sort(x{[!missing]})$ // Remove all missing values from $x$, then sort $x$.

4.  if $i_{half} - n_{NA} < 1$: // Any indices less than 1 lead to a missing value.
...
1.  $i_{half1} := \frac{length(x)}{2}$ // Determine the lower central index of $x$.

2.  $i_{half2} := i_{half1} + 1$ // Determine the upper central index.

3.  $n_{NA} := length(x[missing])$ // Count the missing values in $x$.

4.  $x := sort(x{[!missing]})$ // Remove all missing values from $x$, then sort $x$.

5.  if $i_{half1} - n_{NA} < 1 \lor i_{half2} - n_{NA} < 1$:
...
Why do the algorithms return $NA$ if any offset central index is less than 1? (This is step 4 for odd lengths and step 5 for even lengths.) The median is never known in such cases. Practically speaking, however, the more important reason is that indices must be positive when testing for equality in the final condition. Negative or zero indices are not well defined, and programming languages like R and Python would handle them in unintended ways.

## R details

The R implementation in naidem is more succinct than the above. This plays into the strengths of R, but it makes the code look a little different from these algorithms. Most importantly, the latter are 0-indexed but R is 1-indexed.

You may notice that the check for non-positive indices is the left-hand side of the final conditon. This is appropriate because the right-hand side is actually a condition for returning `NA`, as well. In other words, the right-hand side negates what the equality checks test for, and as a consequence, the function returns a missing value if the condition is `TRUE`. The reason is that the R code combines both algorithms into one, and if the negation returns `FALSE`, the code still needs to check whether `x` was originally even or odd.
